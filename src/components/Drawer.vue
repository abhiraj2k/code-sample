...
export default {
  ...
  methods: {
    ...
    getSnap(top) {
      ...
      const maxPercentHeight =
        (this.containerHeight * (100 - this.maxSpan)) / 100;
  ...
  computed: {
    ...
    getTransitionStyle() {
      return this.getDrawerPosition
        ? this.getBottomTouchStyle
        : this.left
        ? this.getLeftStyle
        : this.right
        ? this.getRightStyle
  ...
  mounted() {
    ...
    document.documentElement.style.overflow =
      this.value == true || this.modelValue == true ? "hidden" : "overlay";
